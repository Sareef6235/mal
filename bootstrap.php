...
function render_header(string $title): void {
    ...
    echo '<style>:root{--bg:#0f172a;--glass:rgba(255,255,255,.14);--line:rgba(255,255,255,.24);--text:#f8fafc;--muted:#cbd5e1}*{box-sizing:border-box}body{margin:0;font-family:Inter,Segoe UI,Arial,sans-serif;color:var(--text);background:linear-gradient(135deg,#0b1023,#16213e,#1d2f5f)}.container{max-width:1180px;margin:16px auto;padding:0 12px 20px}.hero,.card{padding:14px;border-radius:14px;background:var(--glass);border:1px solid var(--line);backdrop-filter:blur(10px)}.card{margin-top:12px}.top-nav{position:sticky;top:0;z-index:100;margin-top:10px;padding:8px;border-radius:12px;background:rgba(15,23,42,.82);border:1px solid rgba(255,255,255,.16)}.top-nav-row{display:flex;gap:8px;align-items:center}.nav-brand{font-weight:700;padding:8px 10px;border-radius:8px;background:rgba(79,70,229,.35);border:1px solid rgba(196,181,253,.35);white-space:nowrap}.nav-toggle{display:none;padding:9px 10px;border-radius:8px;border:1px solid rgba(255,255,255,.2);background:rgba(30,41,59,.75);color:#fff}.menu-search{flex:1;min-width:170px;padding:9px 11px;border-radius:8px;border:1px solid rgba(255,255,255,.2);background:rgba(30,41,59,.72);color:#fff}.menu-groups{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}.menu-group{position:relative}.menu-button{display:flex;align-items:center;gap:7px;padding:9px 12px;border-radius:8px;border:1px solid rgba(196,181,253,.35);background:rgba(124,58,237,.28);color:#fff;cursor:pointer;white-space:nowrap}.menu-button:hover,.menu-button.active{background:rgba(129,140,248,.45)}.dropdown{position:absolute;left:0;top:calc(100% + 6px);min-width:220px;padding:8px;border-radius:10px;background:rgba(15,23,42,.96);border:1px solid rgba(255,255,255,.16);box-shadow:0 12px 24px rgba(2,6,23,.45);opacity:0;visibility:hidden;transform:translateY(6px);transition:opacity .2s ease,transform .2s ease,visibility .2s ease}.dropdown a{display:block;padding:8px 10px;border-radius:8px;text-decoration:none;color:#e2e8f0}.dropdown a:hover,.dropdown a.active{background:rgba(79,70,229,.35);color:#fff}.menu-group:hover .dropdown,.menu-group:focus-within .dropdown,.menu-group.open .dropdown{opacity:1;visibility:visible;transform:translateY(0)}.no-result{display:none;margin-top:8px;color:#fde68a;font-size:13px}input,select,button,textarea{width:100%;padding:10px;border-radius:8px;border:1px solid rgba(255,255,255,.3);background:rgba(15,23,42,.6);color:#fff}button{cursor:pointer;background:linear-gradient(120deg,#4f46e5,#7c3aed)}table{width:100%;border-collapse:collapse;min-width:560px}th,td{padding:9px;border-bottom:1px solid rgba(255,255,255,.18);text-align:left}.small{color:var(--muted)}.msg-ok{color:#86efac}.msg-bad{color:#fecaca}@media (max-width:940px){.nav-toggle{display:inline-flex;width:auto}.menu-groups{display:none;flex-direction:column}.top-nav.mobile-open .menu-groups{display:flex}.dropdown{position:static;display:none;opacity:1;visibility:visible;transform:none;min-width:unset;margin-top:6px}.menu-group.open .dropdown{display:block}.menu-button{width:100%}}</style></head><body><main class="container">';
    echo '<section class="hero"><h1>' . e($title) . '</h1><p class="small">Connected Madrasa Portal Pages</p><nav class="top-nav" id="portal-nav"><div class="top-nav-row"><span class="nav-brand">Madrasa Navigation</span><button class="nav-toggle" type="button" id="nav-toggle" aria-expanded="false" aria-controls="menu-groups">☰ Menu</button><input type="search" id="menu-search" class="menu-search" placeholder="Search menu..." aria-label="Search menu items"></div><div class="menu-groups" id="menu-groups">';
    $menuGroups = [
        ['label' => 'Main Portal', 'icon' => '🏠', 'items' => ['index.php' => 'Portal', 'student_result_viewer.php' => 'Result Viewer']],
        ['label' => 'ERP Dashboard', 'icon' => '📊', 'items' => ['pages/dashboard.php' => 'ERP Dashboard', 'pages/fest_dashboard.php' => 'Fest Dashboard']],
        ['label' => 'Festival Management', 'icon' => '🎉', 'items' => ['pages/festivals.php' => 'Festivals', 'pages/festival_events.php' => 'Fest Events', 'pages/festival_participants.php' => 'Fest Participants', 'pages/festival_scoreboard.php' => 'Fest Scoreboard']],
        ['label' => 'Student Management', 'icon' => '🎓', 'items' => ['pages/students.php' => 'Students', 'pages/subjects.php' => 'Subjects', 'pages/exams.php' => 'Exams', 'pages/results_import.php' => 'Results Import', 'pages/rank_list.php' => 'Rank List', 'pages/marksheet.php' => 'Marksheet']],
        ['label' => 'Attendance', 'icon' => '🕒', 'items' => ['pages/attendance_qr.php' => 'Attendance QR', 'attendance_qr.php' => 'QR Attendance', 'qr_scanner_dashboard.php' => 'Scanner Dashboard', 'scanner.php' => 'Camera Scanner']],
        ['label' => 'Fees', 'icon' => '💳', 'items' => ['pages/fees.php' => 'Fees', 'fee_management.php' => 'Fee Management']],
        ['label' => 'ID Card System', 'icon' => '🪪', 'items' => ['pages/id_card.php' => 'ID Cards', 'idcard.php' => 'ID Card', 'idcard_edit.php' => 'Edit ID Card', 'self_card.php' => 'Self Card', 'self_card_bulk.php' => 'Self Card Bulk', 'id_card_generator_bulk.php' => 'ID Bulk', 'admin_cards.php' => 'ID Admin', 'idcard_bulk.php' => 'A4 ID Print']],
        ['label' => 'Reports', 'icon' => '📑', 'items' => ['reports/result_report.php' => 'Reports', 'class_result_sheet.php' => 'Class Sheet', 'register_result.php' => 'Register Result', 'class_result.php' => 'Class Result']],
        ['label' => 'Admission', 'icon' => '📝', 'items' => ['admission_form.php' => 'Online Admission']],
        ['label' => 'Certificates', 'icon' => '📜', 'items' => ['certificate.php' => 'Certificate']],
        ['label' => 'Database', 'icon' => '🗄️', 'items' => ['database_select.php' => 'DB Select', 'database_mysql.php' => 'MySQL DB', 'database_sqlite.php' => 'SQLite DB']],
        ['label' => 'Admin Tools', 'icon' => '🛠️', 'items' => ['admin_portal.php' => 'Admin Portal', 'admin_bulk_upload.php' => 'Bulk Import']],
    ];

    foreach ($menuGroups as $group) {
        $groupActive = false;
        foreach ($group['items'] as $href => $label) {
            if ($active === basename($href)) {
                $groupActive = true;
                break;
            }
        }
        echo '<div class="menu-group" data-group-label="' . e($group['label']) . '"><button class="menu-button ' . ($groupActive ? 'active' : '') . '" type="button"><span aria-hidden="true">' . e($group['icon']) . '</span><span>' . e($group['label']) . '</span></button><div class="dropdown">';
        foreach ($group['items'] as $href => $label) {
            $isActive = $active === basename($href);
            echo '<a class="menu-item ' . ($isActive ? 'active' : '') . '" href="' . e($href) . '">' . e($label) . '</a>';
        }
        echo '</div></div>';
    }
    echo '</div><p class="no-result" id="no-result">No matching menu items found.</p></nav></section>';
}

function render_footer(): void {
    echo '<script>const nav=document.getElementById("portal-nav");if(nav){const toggle=document.getElementById("nav-toggle"),groups=[...nav.querySelectorAll(".menu-group")],search=document.getElementById("menu-search"),noResult=document.getElementById("no-result");if(toggle){toggle.addEventListener("click",()=>{const open=nav.classList.toggle("mobile-open");toggle.setAttribute("aria-expanded",open?"true":"false");});}groups.forEach((group)=>{const button=group.querySelector(".menu-button");if(!button)return;button.addEventListener("click",(event)=>{if(window.matchMedia("(max-width: 940px)").matches){event.preventDefault();group.classList.toggle("open");}else{groups.forEach((g)=>{if(g!==group)g.classList.remove("open");});group.classList.toggle("open");}});});document.addEventListener("click",(event)=>{if(!nav.contains(event.target)){groups.forEach((group)=>group.classList.remove("open"));}});if(search){search.addEventListener("input",()=>{const term=search.value.trim().toLowerCase();let visibleCount=0;groups.forEach((group)=>{const groupText=((group.dataset.groupLabel||"")+" "+[...group.querySelectorAll(".menu-item")].map((item)=>item.textContent||"").join(" ")).toLowerCase();const show=term===""||groupText.includes(term);group.style.display=show?"":"none";if(show)visibleCount++;if(term!==""&&!show)group.classList.remove("open");});if(noResult)noResult.style.display=visibleCount===0?"block":"none";});}}</script></main></body></html>';
}
