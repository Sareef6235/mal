...
function render_header(string $title): void {
    ...
    echo '<style>:root{--bg:#0f172a;--glass:rgba(255,255,255,.14);--line:rgba(255,255,255,.24);--text:#f8fafc;--muted:#cbd5e1;--accent:#7c3aed}*{box-sizing:border-box}body{margin:0;font-family:Inter,Segoe UI,Arial,sans-serif;color:var(--text);background:linear-gradient(135deg,#0b1023,#16213e,#1d2f5f)}.container{max-width:1180px;margin:16px auto;padding:0 12px 20px}.hero,.card{padding:14px;border-radius:14px;background:var(--glass);border:1px solid var(--line);backdrop-filter:blur(10px)}.card{margin-top:12px}.portal-nav{position:sticky;top:0;z-index:1000;padding:8px 14px;border-radius:14px;background:rgba(9,15,35,.92);border:1px solid rgba(255,255,255,.15);backdrop-filter:blur(8px)}.portal-nav-top{display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{font-size:1.03rem;font-weight:700;letter-spacing:.2px}.menu-toggle{display:none;border:1px solid rgba(255,255,255,.25);background:rgba(124,58,237,.25);color:#fff;border-radius:8px;padding:7px 10px;cursor:pointer}.nav-search input{width:220px;padding:8px 10px;border-radius:8px;border:1px solid rgba(255,255,255,.25);background:rgba(15,23,42,.6);color:#fff}.nav-groups{display:flex;gap:8px;flex-wrap:wrap;list-style:none;margin:10px 0 0;padding:0}.nav-group{position:relative}.group-btn{display:flex;align-items:center;gap:6px;padding:8px 11px;border-radius:8px;text-decoration:none;color:#fff;background:rgba(124,58,237,.2);border:1px solid rgba(196,181,253,.28);font-size:.92rem}.group-btn:hover{background:rgba(124,58,237,.35)}.group-btn.active{outline:2px solid #c4b5fd;background:rgba(124,58,237,.45)}.dropdown{position:absolute;left:0;top:calc(100% + 8px);min-width:220px;background:rgba(11,17,39,.96);border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:8px;list-style:none;margin:0;opacity:0;transform:translateY(8px);visibility:hidden;pointer-events:none;transition:opacity .25s ease,transform .25s ease,visibility .25s}.dropdown a{display:block;padding:8px 10px;border-radius:7px;color:#e2e8f0;text-decoration:none;font-size:.9rem}.dropdown a:hover,.dropdown a.active{background:rgba(124,58,237,.34);color:#fff}.nav-group:hover .dropdown,.nav-group.open .dropdown{opacity:1;transform:translateY(0);visibility:visible;pointer-events:auto}.nav-empty{display:none;padding:8px 10px;color:#fca5a5;font-size:.9rem}input,select,button,textarea{width:100%;padding:10px;border-radius:8px;border:1px solid rgba(255,255,255,.3);background:rgba(15,23,42,.6);color:#fff}button{cursor:pointer;background:linear-gradient(120deg,#4f46e5,#7c3aed)}table{width:100%;border-collapse:collapse;min-width:560px}th,td{padding:9px;border-bottom:1px solid rgba(255,255,255,.18);text-align:left}.small{color:var(--muted)}.msg-ok{color:#86efac}.msg-bad{color:#fecaca}@media (max-width:980px){.menu-toggle{display:block}.nav-search{flex:1}.nav-search input{width:100%}.nav-groups{display:none;flex-direction:column}.portal-nav.open .nav-groups{display:flex}.nav-group{width:100%}.group-btn{justify-content:space-between}.dropdown{position:static;opacity:1;visibility:visible;pointer-events:auto;transform:none;display:none;margin-top:6px}.nav-group.open .dropdown{display:block}}</style></head><body><main class="container">';
    echo '<section class="hero"><h1>' . e($title) . '</h1><p class="small">Connected Madrasa Portal Pages</p>';
    echo '<nav class="portal-nav" id="portalNav"><div class="portal-nav-top"><span class="brand">Madrasa ERP</span><div class="nav-search"><input id="menuSearch" type="search" placeholder="Search menu..."></div><button class="menu-toggle" id="menuToggle" aria-expanded="false">☰ Menu</button></div><ul class="nav-groups" id="navGroups">';
    ...
    foreach ($menuGroups as $group) {
        ...
        echo '<li class="nav-group' . ($groupActive ? ' open' : '') . '"><a class="group-btn' . ($groupActive ? ' active' : '') . '" href="#" aria-expanded="false"><span>' . e($group['icon']) . '</span> ' . e($group['label']) . ' <span>▾</span></a><ul class="dropdown">';
        foreach ($group['items'] as $href => $label) {
            echo '<li><a class="' . ($active === basename($href) ? 'active' : '') . '" href="' . e($href) . '">' . e($label) . '</a></li>';
        }
        echo '</ul></li>';
    }

    echo '</ul><div class="nav-empty" id="navEmpty">No menu item found.</div></nav></section>';
    echo '<script>(function(){const nav=document.getElementById("portalNav");if(!nav)return;const menuToggle=document.getElementById("menuToggle");const groups=[...nav.querySelectorAll(".nav-group")];const search=document.getElementById("menuSearch");const empty=document.getElementById("navEmpty");menuToggle&&menuToggle.addEventListener("click",function(){const open=nav.classList.toggle("open");menuToggle.setAttribute("aria-expanded",open?"true":"false")});groups.forEach(function(group){const btn=group.querySelector(".group-btn");btn&&btn.addEventListener("click",function(e){if(window.matchMedia("(max-width: 980px)").matches){e.preventDefault();group.classList.toggle("open")}})});if(search){search.addEventListener("input",function(){const q=search.value.trim().toLowerCase();let count=0;groups.forEach(function(group){let has=false;group.querySelectorAll(".dropdown a").forEach(function(link){const show=link.textContent.toLowerCase().includes(q)||group.textContent.toLowerCase().includes(q);link.parentElement.style.display=show?"":"none";if(show){has=true;count++}});group.style.display=has?"":"none"});empty.style.display=count?"none":"block"})}})();</script>';
}
